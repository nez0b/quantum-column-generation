% tutorial.tex — End-to-End QCG tutorial (mirrors notebook 06)
...
\setbeamertemplate{headline}{%
  ...
  \hbox{%
    ...
    \begin{beamercolorbox}[wd=.20\paperwidth,ht=2.5ex,dp=1.125ex,right]{palette primary}%
      \raisebox{-0.5ex}{\includegraphics[height=2.5ex]{QCILogoWhite}}\hspace*{6pt}%
    \end{beamercolorbox}}%
}
...
\begin{frame}{Synthetic antenna network (25 antennas in 10\,km box)}
  \centering
  \begin{tikzpicture}[scale=0.7]
    % Sketched scatter of antennas
    \foreach \x/\y in {1/2, 3/3, 5/2, 7/1, 8/4, 6/6, 4/7, 2/6, 1/8,
                       3/9, 5/8, 7/8, 9/7, 1/4, 4/4, 6/4, 8/2, 2/1,
                       5/5, 7/5, 3/1, 9/3, 9/5, 5/9, 2/3} {
      \filldraw[QCIcol1] (\x,\y) circle (2pt);
    }
    \draw[->,thin] (0,0) -- (10.5,0) node[right,font=\footnotesize] {x (km)};
    \draw[->,thin] (0,0) -- (0,10.5) node[above,font=\footnotesize] {y (km)};
    \node[font=\footnotesize] at (5,-0.6) {25 antennas; interference radius 2.5\,km};
  \end{tikzpicture}

  \medskip
  \footnotesize
  Selected by BFS from the highest-degree antenna: a 12-vertex
  connected subgraph used as the head-to-head test case below.
\end{frame}
...
\begin{frame}{Three solvers, one subgraph}
  ...
  \medskip
  All three run on the 12-vertex antenna subgraph; the table on the next
  slide shows the head-to-head result.
\end{frame}
...
\begin{frame}{Key takeaways}
  ...
  \vspace{0.4em}
  \footnotesize
  Notebook: \texttt{notebooks/06\_tutorial\_end\_to\_end.ipynb}\\
